...
const GenreTvSeries = () => {
   ...
   const fetchMovData = async () => {
      ...
      const moviesResponse = await axios.get(
        `https://api.themoviedb.org/3/discover/tv?api_key=${apiKey}&include_adult=false&include_video=false&language=en-US`
      );
         ...
         <Link to={`/tv/${movie.id}`}>
            <div className="relative">
               ...
               <div className="absolute text-white bottom-0 p-2 w-full h-full flex flex-col items-start justify-end transition-opacity">
                  <div className="text-1xl font-bold">{movie?.name}</div>
